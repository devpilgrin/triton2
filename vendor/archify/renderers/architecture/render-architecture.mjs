...
export async function renderArchitecture({ diagram: arch, template = null, outPath = null, sourceEvidence = null } = {}) {
if (!arch || typeof arch !== 'object') throw new Error('renderArchitecture: a diagram object is required');
...
const svg = renderSvg();
if (outPath && template) {
  writeDiagram({ outPath, template, diagramType: 'architecture', meta: arch.meta, svg, cards: arch.cards, sourceEvidence });
}
return { svg, cards: arch.cards, meta: arch.meta };
}
