...
export async function renderSequence({ diagram: sequence, template = null, outPath = null, sourceEvidence = null } = {}) {
if (!sequence || typeof sequence !== 'object') throw new Error('renderSequence: a diagram object is required');
...
const svg = renderSvg();
if (outPath && template) {
  writeDiagram({ outPath, template, diagramType: 'sequence', meta: sequence.meta, svg, cards: sequence.cards, sourceEvidence });
}
return { svg, cards: sequence.cards, meta: sequence.meta };
}
