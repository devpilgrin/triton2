...
import { animateAttr, focusEdgeAttrs, focusNodeAttrs, focusNodeTitle, validateSchema, validateGuidedViews, validateRelationshipIds, writeDiagram, svgAccessibleText, svgRootAttrs } from '../shared/cli.mjs';
...
// Patched for Triton 2: CLI entry replaced by an importable render function.
// The rendering body below is unchanged from the vendored archify 2.15.0 source.
export async function renderLifecycle({ diagram: lifecycle, template = null, outPath = null, sourceEvidence = null } = {}) {
if (!lifecycle || typeof lifecycle !== 'object') throw new Error('renderLifecycle: a diagram object is required');
validateSchema('lifecycle', lifecycle);
validateGuidedViews('lifecycle', lifecycle);
validateRelationshipIds('lifecycle', lifecycle);
validateEngineeringProfile('lifecycle', lifecycle);
...
const svg = renderSvg();
if (outPath && template) {
  writeDiagram({ outPath, template, diagramType: 'lifecycle', meta: lifecycle.meta, svg, cards: lifecycle.cards, sourceEvidence });
}
return { svg, cards: lifecycle.cards, meta: lifecycle.meta };
}
