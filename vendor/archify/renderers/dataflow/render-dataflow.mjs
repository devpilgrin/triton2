...
export async function renderDataflow({ diagram: dataflow, template = null, outPath = null, sourceEvidence = null } = {}) {
if (!dataflow || typeof dataflow !== 'object') throw new Error('renderDataflow: a diagram object is required');
validateSchema('dataflow', dataflow);
validateGuidedViews('dataflow', dataflow);
validateRelationshipIds('dataflow', dataflow);
validateEngineeringProfile('dataflow', dataflow);
...
const svg = renderSvg();
if (outPath && template) {
  writeDiagram({ outPath, template, diagramType: 'dataflow', meta: dataflow.meta, svg, cards: dataflow.cards, sourceEvidence });
}
return { svg, cards: dataflow.cards, meta: dataflow.meta };
}
