...
export async function renderWorkflow({ diagram: workflow, template = null, outPath = null, sourceEvidence = null } = {}) {
if (!workflow || typeof workflow !== 'object') throw new Error('renderWorkflow: a diagram object is required');
validateSchema('workflow', workflow);
validateGuidedViews('workflow', workflow);
validateRelationshipIds('workflow', workflow);
validateEngineeringProfile('workflow', workflow);
...
const svg = renderSvg();
if (outPath && template) {
  writeDiagram({ outPath, template, diagramType: 'workflow', meta: workflow.meta, svg, cards: workflow.cards, sourceEvidence });
}
return { svg, cards: workflow.cards, meta: workflow.meta };
}
